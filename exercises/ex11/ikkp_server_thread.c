/* Code from Head First C modified so it creates a new thread for each connection.
...
int say(int socket, char *s)
{
    ...
    // Modify say so it deliberately causes a segmentation fault:
    // raise(SIGSEGV);
    return res;
}
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        pthread_t tidc;
        int ret = pthread_create(&tidc, NULL, &thread_code, (void*) &connect_d);
        if (ret == -1) {
